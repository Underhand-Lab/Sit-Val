...
const LineupSearchPage: React.FC = () => {
	...
	const [lineups, setLineups] = useState(db.getData('yearlyLineups'));
	...
	return (
		<DataManagementView
			...
			renderItem={(l) => <ListItemCard onClick={() => navigate(`/lineup/${l.id}`)}>{/* 기존 Div 내용을 children으로 전달 */}<Div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><Div style={{ backgroundColor: vars.surface, padding: '4px 8px', borderRadius: '6px', fontSize: '12px', fontWeight: 'bold', color: vars.primary }}>{l.year}</Div><span style={{ fontSize: '16px', fontWeight: 600, color: vars.text }}>{l.name}</span></Div><span style={{ fontSize: '12px', color: vars.text, opacity: 0.4 }}>{l.id}</span></ListItemCard>}
		/>
	);
};
