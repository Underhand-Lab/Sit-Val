...
export interface RunnerInputHandle {
  getAbility: () => RunnerStats;
  getAbilityRaw: () => RunnerStats;
  setData: (newData: RunnerStats) => void;
}

interface RunnerInputProps {
  onDataChange?: (stats: RunnerStats) => void;
  id?: string;
  initialStats?: RunnerStats;
}
 ...
 */
const RunnerInput = forwardRef<RunnerInputHandle, RunnerInputProps>(({ onDataChange, id, initialStats }, ref) => {
  ...
  useImperativeHandle(ref, (): RunnerInputHandle => ({
    getAbility: () => ({ ...stats }),
    getAbilityRaw: () => ({ ...stats }),
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const nextStats = { ...stats, [name]: val } as unknown as RunnerStats;
    setStats(nextStats);
    if (onDataChange) onDataChange(nextStats);
  };

  // 나머지 확률(시도 X 등) 계산 함수
  const getRemainder = (safe: number, out: number): string => (Math.max(0, 1 - safe - out)).toFixed(3);
